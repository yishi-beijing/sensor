...
static rclcpp::Publisher<geometry_msgs::msg::TwistWithCovarianceStamped>::SharedPtr gs_wistWith_pub;
static rclcpp::Publisher<geometry_msgs::msg::PoseWithCovarianceStamped>::SharedPtr gs_pose_pub;
...
static void pose_callback(const geometry_msgs::msg::PoseStamped::ConstPtr msg);
int main(int argc, char **argv)
{
    ...
    auto pvt_source = private_nh->create_subscription<msg_interfaces::msg::Hcinspvatzcb>("/chcnav/devpvt", 10, pvt_callback);
    auto serial_suber = nh->create_subscription<msg_interfaces::msg::HcSentence>("/chcnav/hc_sentence", 10, hc_sentence_callback);
    auto pose_suber = nh->create_subscription<geometry_msgs::msg::PoseStamped>("/sensing/gnss/pose", 1, pose_callback);


    gs_fix_pub = private_nh->create_publisher<sensor_msgs::msg::NavSatFix>("/sensing/gnss/ublox/nav_sat_fix", 10);
    gs_imu_pub = private_nh->create_publisher<sensor_msgs::msg::Imu>("/sensing/imu/tamagawa/imu_raw", 10);
    gs_Orientation_pub=private_nh->create_publisher<autoware_sensing_msgs::msg::GnssInsOrientationStamped>("/autoware_orientation", 10);
    gs_wistWith_pub=private_nh->create_publisher<geometry_msgs::msg::TwistWithCovarianceStamped>("/gs_wistWith", 10);
    gs_pose_pub=private_nh->create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>("/localization/pose_estimator/pose_with_covariance", 10);

    gs_devpvt_pub = nh->create_publisher<msg_interfaces::msg::Hcinspvatzcb>("/chcnav/devpvt", 10);
    gs_devimu_pub = nh->create_publisher<msg_interfaces::msg::Hcrawimub>("/chcnav/devimu", 10);
...
static void pvt_callback(const msg_interfaces::msg::Hcinspvatzcb::ConstPtr msg)
{
    ...
    fix.altitude = msg->altitude+9.25695118661424;
    ...
    tf2_quaternion.setRPY(msg->roll / 180 * M_PI, -msg->pitch / 180 * M_PI, yaw / 180 * M_PI);
    tf2::convert(tf2_quaternion, geometry_quaternion);
    gs_Orientation.header=msg->header;
    gs_Orientation.header.stamp=rclcpp::Clock().now();
    gs_Orientation.header.frame_id="map";
    ...
    geometry_msgs::msg::TwistWithCovarianceStamped twiistWith_msg;
    twiistWith_msg.header= msg->header;
    imu.header.stamp = rclcpp::Clock().now();
    
    twiistWith_msg.twist.twist.linear.x = msg->enu_velocity.x;  // 设置线性速度 (m/s)
    
    twiistWith_msg.twist.twist.linear.y = msg->enu_velocity.y;  // 设置线性速度 (m/s)

    twiistWith_msg.twist.twist.linear.z = msg->enu_velocity.z;  // 设置线性速度 (m/s)

    twiistWith_msg.twist.twist.angular = msg->vehicle_angular_velocity;
    // 设置角速度 (rad/s)

        // 填充协方差矩阵
    twiistWith_msg.twist.covariance[0] = 0.01;  // 协方差矩阵 (vx, vx)
    twiistWith_msg.twist.covariance[1] = 0.0;   // 协方差矩阵 (vx, vy)
    twiistWith_msg.twist.covariance[5] = 0.0;   // 协方差矩阵 (vy, vy)
    twiistWith_msg.twist.covariance[14] = 0.01; // 协方差矩阵 (angular velocity)
    gs_wistWith_pub->publish(twiistWith_msg);
